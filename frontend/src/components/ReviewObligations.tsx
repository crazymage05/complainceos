...
export default function ReviewObligations() {
    ...
    return (
      <div className="min-h-screen bg-canvas flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-inkMute text-sm font-medium">Loading your compliance profile…</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="relative min-h-screen bg-canvas overflow-hidden">
      <div className="pointer-events-none absolute -top-40 left-1/2 -translate-x-1/2 w-[700px] h-[500px] rounded-full bg-accent/[0.06] blur-[140px]" />
      <div className="relative max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-ink">Review Your Compliance DNA</h1>
          <p className="text-inkMute mt-1 text-sm max-w-md mx-auto">
            AI identified <span className="font-semibold text-accent-soft">{totalCount} obligations</span> for <span className="font-semibold text-inkSoft">{businessName}</span>.
            Uncheck anything that doesn't apply and set your own due dates.
          </p>
        </div>

        {/* Progress bar */}
        <div className="mb-6 os-panel p-4 flex items-center justify-between gap-4">
          <div className="flex-1">
            <div className="flex justify-between text-xs font-semibold text-inkMute mb-1.5">
          ...
          <button
            ...
            className="os-btn-accent flex-shrink-0 px-5 py-2.5 text-sm flex items-center gap-2"
          >
            {confirming ? (
        ...
        <div className="space-y-3">
          {Object.entries(grouped).map(([cat, obs]) => {
            ...
            return (
              <div key={cat} className="os-card overflow-hidden">
                {/* Category header */}
                <div
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-inkSoft text-sm">{cat}</span>
                    <span className="text-xs px-2 py-0.5 bg-white/5 text-inkMute rounded-full font-medium">
                      {obs.filter((o) => checked.has(o.instance_id)).length}/{obs.length}
                    </span>
                  </div>
                  <svg
                    className={clsx('w-4 h-4 text-inkFaint transition-transform', expanded && 'rotate-180')}
                ...
                {/* Obligation rows */}
                {expanded && (
                  <div className="border-t border-edge divide-y divide-edgeSoft">
                    {obs.map((o) => {
                      ...
                      return (
                        <div
                          ...
                          className={clsx(
                            ...
                            isChecked ? 'bg-transparent' : 'bg-white/[0.02] opacity-50'
                          )}
                        >
                          <input
                            type="checkbox"
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={clsx(
                              ...
                              isChecked ? 'text-ink' : 'text-inkFaint line-through'
                            )}>
                              {o.obligation_name}
                            </p>
                            <p className="text-xs text-inkFaint mt-0.5 capitalize">
                              {o.period} · {o.description || 'Statutory obligation'}
                            </p>
                          </div>
                          {isChecked && (
                            <div className="flex items-center gap-1.5 flex-shrink-0">
                              <span className="text-xs text-inkFaint hidden sm:block">Due:</span>
                              <input
                                type="date"
                                value={dueDates[o.instance_id] ?? ''}
                                onChange={(e) => setDate(o.instance_id, e.target.value)}
                                className="os-input text-xs px-2 py-1 w-32 [color-scheme:dark]"
                              />
                            </div>
                          )}
                        </div>
        ...
        <div className="mt-6 flex justify-center">
          <button
            onClick={handleConfirm}
            disabled={confirming || selectedCount === 0}
            className="os-btn-accent px-8 py-3.5 text-base flex items-center gap-2"
          >
            {confirming ? (
              <><div className="w-5 h-5 border-2 border-canvas border-t-transparent rounded-full animate-spin" /> Saving your compliance profile…</>
            ) : (
        ...
        <p className="text-center text-xs text-inkFaint mt-3">
